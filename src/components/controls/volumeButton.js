...
const VolumeButton = ({ songAudioRef }) => {
    ...
    const [mute, setMute] = useState(storage.get(storageKey.MUTE));
    const volumnProcessElm = useRef();

    useEffect(() => {
        const currentVolume = storage.get(storageKey.VOLUME) || 1;
    ...
    useEffect(() => {
        if (mute) {
            ...
            const volume = storage.get(storageKey.VOLUME) || 1;
    ...
    const handleVolumnSliderMouseUp = useCallback((width) => {
        ...
        songAudioRef.current.volume = volume;
        storage.set(storageKey.VOLUME, volume);
    }, [songAudioRef])

    const handleToggleVolume = (e) => {
        if (e.target.closest('.icon-btn')){
            setMute(!mute);
            storage.set(storageKey.MUTE, !mute);
        }
    };
        ...
        <VolumeContainer>
            ...
            <VolumnSlider>
                <Slider 
                    ...
                    current={ storage.get(storageKey.VOLUME) || 1 }
